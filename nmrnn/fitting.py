...
from nmrnn.rnn_code import batched_nm_rnn_loss, nm_rnn, batched_lr_rnn_loss, lr_rnn, \
    batched_nm_rnn_loss_frozen, lin_sym_nm_rnn, batched_lin_sym_nm_rnn_loss, context_nm_rnn, \
        batched_context_nm_rnn_loss, batched_context_nm_rnn_loss_frozen,batched_lr_rnn_loss_split, \
            lstm_batched_loss, batched_rnn_loss, batched_rnn_loss_split, batched_lstm_loss_split
...
##### LSTM FITTING
def fit_lstm_mwg(inputs, targets, params, optimizer, init_carry, num_iters, 
                 wandb_log=False, batch=False, batch_size=100, keyind=13):
    opt_state = optimizer.init(params)
    (c0, h0) = init_carry
    N_data = inputs.shape[0]
    key = jr.PRNGKey(keyind)
    ...
    def _step(params_and_opt, input):
        ...
        if batch: 
            ...
            batch_inputs = inputs
            batch_targets = targets
        loss_value, grads = jax.value_and_grad(lstm_batched_loss)(params, c0, h0, batch_inputs, batch_targets)
        updates, opt_state = optimizer.update(grads, opt_state, params)
        params = optax.apply_updates(params, updates)
        return (params, opt_state, key), (params, loss_value)
    ...
    best_params = params
    for n in range(num_iters//1000):
        (params,_,_), (_,loss_values) = lax.scan(_step, (params, opt_state, key), None, length=1000)
...
def fit_lstm_inputweights_only(inputs, targets, input_params, other_params, optimizer, init_carry, num_iters,
    ...
    def _step(params_and_opt, input):
        ...
        if batch: 
            ...
            batch_inputs = inputs
            batch_targets = targets
        loss_value, grads = jax.value_and_grad(batched_lstm_loss_split)(input_params, other_params, c0, h0, batch_inputs, batch_targets)
        updates, opt_state = optimizer.update(grads, opt_state, input_params)
        input_params = optax.apply_updates(input_params, updates)
        return (input_params, opt_state, key), (input_params, loss_value)
